...
from app import sum, difference, multiply, division
...
def subtraction():
    if request.method == 'POST':
        a = request.args.get('first')
        b = request.args.get('second')
        return str(difference(int(a), int(b)))
...
def multiplication():
    if request.method == 'POST':
        a = request.args.get('first')
        b = request.args.get('second')
        return str(multiply(int(a), int(b)))
...
def division():
    if request.method == 'POST':
        a = request.args.get('first')
        b = request.args.get('second')
        return str(division(int(a), int(b)))

@app.route("/")
def home():
    return """For Addition go to \sum.\n
    For Subtraction go to \diff.\n
